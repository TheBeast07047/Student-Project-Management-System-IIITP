...
const Navbar = ({ userRole: propUserRole = null, user: propUser = null, roleData: propRoleData = null }) => {
  ...
  const getNavigationItems = () => {
    ...
    if (userRole === 'student') {
      ...
      // Sem 5: Faculty Allocation Preferences
      if (currentSemester === 5) {
        const sem5Group = sem5Data?.sem5Group;
        // Show "Faculty Preferences" link when student is in a finalized group
        if (sem5Group && (sem5Group.status === 'finalized' || sem5Group.status === 'complete')) {
          items.push({
            name: 'Faculty Preferences',
            path: '/student/allocation/preferences'
          });
        }
      }
    ...
    if (userRole === 'faculty') {
      items.push(
        ...
        { name: 'Allocation Rankings', path: '/faculty/allocation/rankings' }
      );
    }
      ...
      items.push(
        ...
        },
        {
          ...
          items: [
            ...
            { name: 'Faculty Allocation', path: '/admin/allocation' }
          ]
        }
      );
    }
