...
// Allocation API — Gale-Shapley Faculty Allocation System
export const allocationAPI = {
  // Public
  getAvailableFaculty: () => api.get('/allocation/faculty-list'),

  // Admin endpoints
  admin: {
    getConfig: (params = {}) => {
      const queryString = new URLSearchParams(params).toString();
      return api.get(`/allocation/admin/config${queryString ? '?' + queryString : ''}`);
    },
    updateConfig: (config) => api.put('/allocation/admin/config', config),
    validate: (data = {}) => api.post('/allocation/admin/validate', data),
    run: (data = {}) => api.post('/allocation/admin/run', data),
    confirm: (runId) => api.post('/allocation/admin/confirm', { runId }),
    reset: (runId) => api.post('/allocation/admin/reset', { runId }),
    getResults: (params = {}) => {
      const queryString = new URLSearchParams(params).toString();
      return api.get(`/allocation/admin/results${queryString ? '?' + queryString : ''}`);
    },
    getAdminPool: (params = {}) => {
      const queryString = new URLSearchParams(params).toString();
      return api.get(`/allocation/admin/admin-pool${queryString ? '?' + queryString : ''}`);
    },
    assignAdminPoolGroup: (groupId, facultyId, runId) =>
      api.post(`/allocation/admin/admin-pool/${groupId}/assign`, { facultyId, runId }),
    getFacultyCapacity: (params = {}) => {
      const queryString = new URLSearchParams(params).toString();
      return api.get(`/allocation/admin/faculty-capacity${queryString ? '?' + queryString : ''}`);
    },
    updateFacultyCapacity: (facultyId, capacity) =>
      api.put(`/allocation/admin/faculty/${facultyId}/capacity`, { capacity }),
  },

  // Student endpoints
  student: {
    getConfig: () => api.get('/allocation/student/config'),
    getPreferences: () => api.get('/allocation/student/preferences'),
    submitPreferences: (preferences) => api.post('/allocation/student/preferences', { preferences }),
    getDemand: () => api.get('/allocation/student/demand'),
    getStatus: () => api.get('/allocation/student/status'),
  },

  // Faculty endpoints
  faculty: {
    getGroupsToRank: () => api.get('/allocation/faculty/groups-to-rank'),
    submitRankings: (rankings) => api.post('/allocation/faculty/rankings', { rankings }),
    getStatus: () => api.get('/allocation/faculty/status'),
  },
};
