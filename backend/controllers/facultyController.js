...
const getUnallocatedGroups = async (req, res) => {
  try {
    ...
    const groups = activePreferences.map(pref => {
      ...
      // Check if it is the logged in faculty's turn
      const facultyPref = pref.preferences?.find(p => p.faculty?._id?.toString() === facultyId.toString() || p.faculty === facultyId.toString() || p.faculty?.toString() === facultyId.toString());
      const facultyPriority = facultyPref ? facultyPref.priority : null;
      let currentIndex = pref.currentFacultyIndex || 0;
      if (pref.project && !pref.group) {
        currentIndex = pref.project.currentFacultyIndex || 0;
      }
      const isCurrentTurn = facultyPriority === (currentIndex + 1);
      ...
      return {
        ...
        currentPreference: currentIndex + 1,
        isCurrentTurn: isCurrentTurn,
        facultyPriority: facultyPriority,
